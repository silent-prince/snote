function createSentElement(message,replymessage,aarumi_from,replyingmessageid){
    let tempid = "temp" + Math.floor(1000 + Math.random() * 9000);  
    return createElement(tempid,message,replymessage,aarumi_from,"sent","sending","new",replyingmessageid);
}
function createElement(aarumiId,message,replyMessage,aarumi_from,type,status,newOrOld,replyingmessageid){
    let replyHtml = "";
...
function createReceivedElement(data){
    createElement(data.aarumiId,data.message,data.replyMessage,data.replyFrom,"received","none","old","00");
}
function createReceivedElement2(data){
    createElement(data.id,data.message,data.replyMessage,data.replyFrom,"received","none","old","00");
}
