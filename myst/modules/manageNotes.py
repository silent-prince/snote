...
def notes_page(request):
    ...
    #whois_id = request.session.get("whois_id")
    #if not whois_id:
        #return redirect("notes_login")  # If not logged in, redirect to loginx
    #whois = Whois.objects.get(name=whois_id)
...
def get_notes(request):
    ...
    if request.method == "POST":
        try:
            ...
            return JsonResponse({"success": True,"notes": list(notes.values())},status=200)
...
def add_note(request):
    if request.method == "POST":
        try:
            ...
            if(mode=="secret"):
                note=Note.objects.create(whois=whois,username=user.username, content=content,mode=mode)
